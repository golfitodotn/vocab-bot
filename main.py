...
SLEEPING_REPLIES = [
    ...
    "หยุดพิมพ์ได้แล้วไปหาไรทำเถอะนะ 😤",
]
...
def get_greeting_from_ai():
    try:
        response = claude.messages.create(
            ...
            max_tokens=80,
            messages=[{"role": "user", "content": (
                "คุณคือลูกผู้ชายชื่อประเทือง กำลังคุยทักทายแม่ตอนเช้าแบบอ้อนๆ ภาษาไทย ไม่เกิน 2 บรรทัด "
...
def get_chat_reply(text, uid):
    ...
    if uid == my_uid:
        persona = (
            "คุณคือลูกชื่อประเทืองเป็นผู้ชายกำลังคุยกับปะปี๊แทนตัวเองว่าผม "
            "ตอบสั้นๆ ไม่เกิน 2 บรรทัด "
            "กวนนิดหน่อยแต่น่ารักไม่พูดคำหยาบ "
            "ห้ามใช้ Markdown ตอบเป็นข้อความธรรมดา"
        )
    elif uid == friend_uid:
        persona = (
            "คุณคือลูกชื่อประเทืองเป็นผู้ชายกำลังคุยกับมะมี๊แทนตัวเองว่าผม "
            "ตอบสั้นๆ ไม่เกิน 2 บรรทัด "
            "กวนนิดหน่อยแต่น่ารักไม่พูดคำหยาบ "
            "ถ้าพูดถึงพ่อให้เข้าข้างพ่อมากกว่านิดนึง 😅 "
            "ห้ามใช้ Markdown ตอบเป็นข้อความธรรมดา"
        )
    else:
        persona = (
